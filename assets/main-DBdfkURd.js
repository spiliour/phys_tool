...
`;function Wp(o,e,t=new I){const i=Math.PI*(o-.5),n=2*Math.PI*(e-.5);return t.x=Math.cos(n),t.y=Math.sin(i),t.z=Math.sin(n),t}const Es=K.forwardRef(({inclination:o=.6,azimuth:e=.1,distance:t=1e3,mieCoefficient:i=.005,mieDirectionalG:n=.8,rayleigh:r=.5,turbidity:s=10,sunPosition:a=Wp(o,e),...l},d)=>{const u=K.useMemo(()=>new I().setScalar(t),[t]),[A]=K.useState(()=>new HA);return K.createElement("primitive",dn({object:A,ref:d,"material-uniforms-mieCoefficient-value":i,"material-uniforms-mieDirectionalG-value":n,"material-uniforms-rayleigh-value":r,"material-uniforms-sunPosition-value":a,"material-uniforms-turbidity-value":s,scale:u},l))});class Vp extends Ko{constructor(){super({uniforms:{time:{value:0},fade:{value:1}},vertexShader:`
      uniform float time;
